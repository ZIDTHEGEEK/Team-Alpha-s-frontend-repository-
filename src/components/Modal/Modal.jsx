...
const Modal = ({ setOnClick }) => {
  const [status, setStatus] = useState("clicked");
  const [address, setAddress] = useState(
    "0xgeeg63gwg72191dt4gr45hgg5gh4hy456ye76",
  );
          ...
          {status === "processing" && (
            <div className='loading'>
              <div className='loading-dot'>
                <div
                  className={`dot ${activeIndex === 0 ? "active" : ""}`}></div>
                <div
                  className={`dot ${activeIndex === 1 ? "active" : ""}`}></div>
                <div
                  className={`dot ${activeIndex === 2 ? "active" : ""}`}></div>
              </div>
            ...
            <div className='connected-body'>
              ...
              <div className="view-token">
               <div className="copy">
                ...
                <p>Copy Address</p></div>
               <div className="copy">
                <CgNotes color="gray"/>
                <p>View On Explorer</p></div>
             
              </div>
            </div>
            <div className='connected-footer'>
              <p>Connected with Meta mask</p>
              <button className='disconnected' onClick={()=>setStatus('clicked')}>Disconnect</button>
            </div>
          </div>
        )}
        {status === "failed" && (
